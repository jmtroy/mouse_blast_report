...
# "." > 5		value
# col 5 > 6		strand
# col 7 > 7		annotation (from Homer)
TMP_ANNO_FILE=$OUTPUT_DATA_FOLDER/tmp_homer_anno.bed
tail -n +2 $ANNO_INPUT_FILE | awk -F"\t" -v OFS="\t" '{$2, $3, $4, $1, ".", $5,  $7}' > $TMP_ANNO_FILE
...
grep -v "^#" $PEAK_INPUT_FILE | awk -F"\t" -v OFS="\t" '{$2, $3, $4, $1, ".", $5,  $8, $11, $12}' > $TMP_PEAK_FILE
